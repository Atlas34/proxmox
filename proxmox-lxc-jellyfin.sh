...
function default_settings() {
        ...
        echo -e "${DGN}Using CT Type ${BGN}Privileged${NORMAL}"
        CT_TYPE="0"
...
function advanced_settings() {
        ...
        echo -e "${YELLOW}Type ${CROSS}${YELLOW}Unprivileged, or Press [ENTER] for Default: Privileged"
        read CT_TYPE1
        ...
        CT_TYPE="1"
        echo -en "${DGN}Set CT Type ${BLUE}Unprivileged${NORMAL}"  
        fi;
        echo -e " ${CM}${NORMAL} \r"
        sleep 1
        clear
        ...
        echo -e "${YELLOW}Enter a Static IPv4 CIDR Address(xxx.xxx.xxx.xxx/24), or Press [ENTER] for Default: DHCP "
        read NET
...
lxc-attach -n $CTID -- bash -c "$(wget -qLO - https://raw.githubusercontent.com/atlas34/proxmox/proxmox-jellyfin-install.sh)" || exit
